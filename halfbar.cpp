...
struct Image
{
  ...
  bool isMasked { false };  // Enable or disable masking based on command line arguments
};

std::string getArr(std::vector<unsigned char>& arr, int pos, char i)
{ // Convert array to string and insert character before values
  std::string buf {};
  for (int x {0}; x < 3; x++)
...
{ // convert image to terminal pixels
  ...
  if (img.v%2 == 0)
  { // Check if vertical resolution is even
  ...
  { // If vertical resolution is odd, generate top line with bottom pixels
